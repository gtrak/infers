...
Applied in 4 GEMM calls across `forward()` and `forward_paged()` prefill paths. The decode path uses `m=1` so column-major and row-major are identical for single-row output — correct by coincidence. After applying, prefill attention verification improved from cos=0.008 to cos>0.9996 across all 34 stages (attention + MLP) on both GPUs. See [[crates/backends/native/src/attention/flat_forward.rs#forward]], [[crates/backends/native/src/attention/paged_forward.rs#forward_paged]].
...
See [[crates/backends/native/src/attention/paged_forward.rs#forward_paged]], [[crates/backends/native/src/attention/paged_decode.rs#decode_forward_paged]].
...
- OxideKernels loading `oxide_kernels.cubin` with typed launch wrappers via the oxide bridge module (41 kernels: rmsnorm, silu, silu_glu, attn_output_gate, rope, embedding_gather, add, argmax_bf16, softmax, kv_cache, paged_kv_write, paged_kv_read, gdn_recurrent_step, gdn_gated_delta_prefill, gdn_gated_delta_update, gdn_chunked_gated_delta_prefill, gdn_mamba2_prefill, gdn_mamba2_update, conv1d_depthwise_silu, rms_norm_gated, paged_attention_decode, fp8_quantize, fp8_dequantize, int4_gemm_ksplit, int4_gemm_v3_ksplit, int4_gemm_warp, int4_gemm_warp_split, reduce_partial_sums_bf16, int4_dequant_to_bf16, nvfp4_dequant_to_bf16, nvfp4_gemm_fused, nvfp4_gemm_fused_ksplit, nvfp4_gemm_v3_ksplit, sanitize_nan_bf16, bf16_gemm_tiled) — cubin is a build artifact regenerated by `cargo oxide build` + `--save-cubin` in `crates/cuda-oxide-kernels/`, not tracked in git (`.gitignore` excludes `*.cubin`).
...
One `OxideKernels` instance per GPU loads the same cubin (`oxide_kernels.cubin`) on each device's primary context. The old `KernelRegistry` system was removed — kernels go through oxide bridge's typed launch methods. See [[crates/cuda/src/oxide_bridge.rs#OxideKernels]].
