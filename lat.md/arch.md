...
### Oxide Bridge: Launch Wrapper Methods (34 Kernels)

All 34 launch wrapper methods on `OxideKernels` use typed module dispatch via `KernelModules`. No remaining `raw_launch` callers.

### Oxide Bridge: Typed Module Dispatch Pattern

Dispatch via `KernelModules` avoids manual argument packing by calling type-safe methods generated by `#[cuda_module]`. Migration pattern:

1. Capture lengths before borrowing slices (avoids borrow conflicts with `device_ptr_mut`).
2. Extract device pointers from cudarc buffers — guards keep the underlying memory alive.
3. Wrap pointers in temporary `DeviceBuffer::from_raw_parts(ptr, len, ctx)` — unsafe but safe because CudaSlice owns the memory.
4. Call the typed module method (e.g., `self.modules.common.infers_add_bf16(...)`) with the DeviceBuffers.
5. `std::mem::forget()` the DeviceBuffers so their destructors do not free cudarc-owned memory.

Error mapping: `DriverError` from the typed method is wrapped in `anyhow::anyhow!()`. Guards (`_a_g`, `_b_g`, `_o_g`) remain in scope through the kernel launch to prevent early drop.
...
| `launch_int4_gemm_warp` | `int4_gemm_warp` | grid=(ceil(n/8), 1, 1), block=(32,8,1), smem=0 — warp-cooperative: each warp computes one column via shuffle reduction, lane 0 writes output directly (no partial_sums) |
...
| `launch_nvfp4_gemm_v3_ksplit` | `nvfp4_gemm_v3_ksplit` | grid=(ceil(n/64), k_split, 1), block=(64,1), smem=0 — v3 bandwidth-optimized NVFP4 K-split: same 4 optimizations as INT4 v3 applied to NVFP4 dequant (group_size=16). Production kernel for NVFP4 decode |
