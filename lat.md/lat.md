...
Pre-allocated K-split partial sums buffers in `DecodeWorkspace` eliminating per-token dynamic allocations during GEMM dispatch.

Two separate buffers: (1) `partial_sums: CudaSlice<f32>` (K_SPLIT × sharded_intermediate, ~1.4MB) for decode-loop GEMMs. (2) `lm_head_partial_sums: CudaSlice<f32>` (K_SPLIT × vocab_size, ~12MB on RTX 5090) for the LM head GEMM — previously passed `None` triggering a ~12MB allocation every step. Both threaded via `partial_sums_buf`. Decode passes `Some(&mut ws.partial_sums)` or `Some(&mut ws.lm_head_partial_sums)`, prefill passes `None`. Eliminates ~801 alloc_zeros per token including the LM head.
...
New `attn_out` field provides a shared output buffer for both GDN and attention decode outputs, eliminating the `attn_outputs: Vec<CudaSlice>` and its per-layer allocation. The `gdn` field holds a nested `GdnWorkspace` with 15 pre-allocated buffers covering all GDN intermediate allocations (mixed_qkv, conv_input, conv_out, query/key/value, expanded heads, projection outputs, recurrent step output, z-gate, norm output, and fallback zero buffers for a_log/dt_bias). The conv_out_last buffer was eliminated in EXP-015 — q/k/v splits now copy directly from conv_out using offsets. See [[crates/backends/native/src/workspace.rs#GdnWorkspace]].
