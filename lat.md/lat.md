...
- **compare.py**: CLI entry point for stage-by-stage comparison. Resolves phase-aware dump directory paths: when `--dump-dir` is a layer_N directory it appends `{phase}/`, when it is a root dump directory it inserts `layer_{N}/{phase}/`. Accepts `--phase prefill|decode` to select which phase subdirectory to compare against (default: decode). Discovers engine dumps, loads DumpConfig, creates WeightLoader, and chains stages together computing reference outputs from `inputs` dict (each stage feeds the next). Intermediate results are always stored with `_gpu{idx}` suffix (e.g. `"attn.q_proj_raw_gpu0"`, `"attn.q_proj_raw_gpu1"`) so that each GPU's reference is computed independently — GPU 1 reads its own `q_proj_raw_gpu1` rather than reusing GPU 0's value. Filters by layer type (full-attention vs GDN) and category (`--stages attn mlp gdn final`). Returns exit code 0 if all pass, 1 if any fail.
...
- **attention.py**: Full-attention stages — QProjRaw (doubled for gate), QNorm (per-head RMSNorm), Gate (extracted from q_proj_raw), KProj, KNorm, VProj, AttentionCombined (RoPE + GQA expand + causal SDPA), Gated (combined * sigmoid(gate)), OProj. Includes helpers: `_apply_rope`, `_build_rope_cache`, `_gqa_repeat_kv`, `_scaled_dot_product_attention` (applies causal mask via upper-triangular fill for prefill, degenerates to no-mask for decode with seq_len=1).
