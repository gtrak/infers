...
#### Decode RoPE Position Mismatch (INVESTIGATED)

Decode RoPE uses **absolute position** (token's 0-indexed position in the full sequence), computed as `session.tokens.len().saturating_sub(1)`. The Python reference uses `arange(seq_len)` which gives position=0 for decode — a mismatch.

The RoPE implementation ([[crates/backends/native/src/rope.rs#precompute_rope_tables]], rope.cu kernel) is **correct**. Engine output matches position=44 with bf16 precision (max_diff=3.9e-3, cos_sim=0.999997), while position=15 gives max_diff=1.26. Prefill RoPE is correct (positions 0-14 match). Fix: Python reference must use absolute token position for decode, not `arange(seq_len)`.
...
The reference comparison code in [[tests/compare/stages/attention.py]] had the same interleaved-vs-contiguous mismatch: `QNormStage` and `GateStage` assumed contiguous `[Q_all, G_all]` layout (taking first/second halves), producing near-zero cosine similarity (~0.12 for Q-norm, ~0.61 for gate). Fixed by extracting from interleaved offsets matching the engine code.
## Remaining
...
RS:
XN:The `infer` binary at [[crates/backends/native/src/bin/infer.rs]] is a one-shot inference tool that loads a model, runs prefill + decode, and prints generated text. It follows the same pattern as the smoke test: load config, shard weights via TP, initialize CUDA contexts, create ForwardEngine, initialize paged KV cache, tokenize, build SamplingConfig from CLI args, then prefill + decode with the configured sampling strategy.
TX:
BY:CLI arguments are parsed via clap: `--model` (required), `--tp` (default 2), `--prompt` (optional), `--token-ids` (comma-separated IDs, bypasses tokenizer), `--max-tokens` (default 64), `--dump-dir` (enables probe dumps), `--dump-layers` (layer filter for dumps), `--group-size` (quantization group size, default 128), `--max-seq-len` (override sequence length), `--no-chat` (disables chat template wrapping), and sampling flags: `--temperature` (default 1.0), `--top-k` (default 0 = disabled), `--top-p` (default 1.0), `--repetition-penalty` (default 1.0), `--presence-penalty` (default 0.0), `--frequency-penalty` (default 0.0), `--seed` (optional, for deterministic sampling), and `--eos-token-id` (optional). Either `--prompt` or `--token-ids` must be specified. Sampling strategy is determined by CLI flags: temperature 0 → Greedy, top_k > 0 → TopK, top_p < 1.0 → TopP, temperature != 1.0 → Temperature, otherwise Greedy. RNG seeding uses the provided seed or falls back to system entropy via `sample::random_seed()`.
...
- **io.py**: bf16 raw I/O (`load_raw_bf16`, `save_raw_bf16`), metadata loading (`load_meta`), and dump discovery (`discover_dumps`, `discover_final_dumps`). `discover_dumps` recursively searches layer_N subdirectories (including `prefill/` and `decode/`) for `.meta` files; each meta_dict includes a `phase` field distinguishing prefill from decode dumps.
...
- **compare.py**: CLI entry point for stage-by-stage comparison. Resolves phase-aware dump directory paths: when `--dump-dir` is a layer_N directory it appends `{phase}/`, when it is a root dump directory it inserts `layer_{N}/{phase}/`. Accepts `--phase prefill|decode` to select which phase subdirectory to compare against (default: decode). Supports `--mode custom` (default, Python reference stages) and `--mode oracle` (HF oracle dumps). Discovers engine dumps, loads DumpConfig, creates WeightLoader, and chains stages together computing reference outputs from `inputs` dict (each stage feeds the next). The `inputs` dict also carries `dump_dir` for KV cache path resolution during decode attention. Intermediate results are always stored with `_gpu{idx}` suffix (e.g. `"attn.q_proj_raw_gpu0"`, `"attn.q_proj_raw_gpu1"`) so that each GPU's reference is computed independently — GPU 1 reads its own `q_proj_raw_gpu1` rather than reusing GPU 0's value. Filters by layer type (full-attention vs GDN) and category (`--stages attn mlp gdn final`). Returns exit code 0 if all pass, 1 if any fail.

Stage modules under `tests/compare/stages/`:

- **base.py**: `Stage` abstract class with `compute(inputs, weights, config, layer_idx, gpu_idx)` and `compare(dump_dir, ref, layer_idx, gpu_idx)` methods. The `inputs` dict carries previously computed reference outputs so stages can chain. Also provides `_get_input(inputs, key, gpu_idx)` helper for GPU-specific key resolution — all intermediate outputs are stored with `_gpu{idx}` suffix (e.g. `"attn.q_proj_raw_gpu1"`), and this helper resolves the correct key regardless of whether the value is shared across GPUs or GPU-specific.
...
- **attention.py**: Full-attention stages — QProjRaw (doubled for gate), QNorm (per-head RMSNorm), Gate (extracted from q_proj_raw), KProj, KNorm, VProj, AttentionCombined (prefill: RoPE + GQA expand + causal SDPA; decode: loads k_cached/v_cached from prefill dump dir, concatenates with decode token, applies RoPE to Q only — K already has RoPE baked in), Gated (combined * sigmoid(gate)), OProj. Includes helpers: `_apply_rope`, `_build_rope_cache`, `_gqa_repeat_kv`, `_scaled_dot_product_attention` (applies causal mask via upper-triangular fill for prefill, no-mask for decode with seq_len=1).
...
### HF Oracle Comparison

Replaces custom Python reimplementations with HuggingFace as ground-truth oracle. The custom stages had bugs (wrong RMSNorm formula) that matched the engine's bugs, producing false "ALL STAGES PASSED" results.

- **oracle.py**: Runs the HF model via `AutoModelForCausalLM.from_pretrained()` and captures intermediate tensors via forward hooks on each decoder layer. Captures: `norm1_input`, `norm1_output`, `attn_output` (from `self_attn` or `linear_attn`), `norm2_input`, `norm2_output`, `mlp_output`, `layer_output`. Saves prefill and decode phases as `.pt` files under `{output_dir}/layer_{i}/{phase}/`. Returns a summary dict with shapes and layer types. CLI: `python -m tests.compare.oracle --model-dir /path --token-ids 1,2,3 --output-dir /tmp/oracle`.
- **hf_compare.py**: Compares HF oracle `.pt` dumps against engine `.raw` bf16 dumps. Strips engine prefixes (`attn.`, `gdn.`) and GPU suffixes to map to oracle tensor names. Prints per-layer table with cosine similarity, L2 error, max_diff. Identifies first divergence point. CLI: `python -m tests.compare.hf_compare --oracle-dir /tmp/oracle --engine-dir /tmp/dump --phase prefill`.
- **compare.py --mode oracle**: Integrates oracle comparison into the main CLI. `--mode custom` (default) uses Python reference stages; `--mode oracle` uses HF oracle dumps. Accepts `--oracle-dir` and `--engine-dir` for oracle mode, `--threshold` for cosine similarity cutoff.
...
### GDN TP=2 Comparison Investigation
...
4. **Sampling config**: Builds `SamplingConfig` from request parameters: determines sampling strategy based on `top_k` → `TopK`, `top_p < 1.0` → `TopP`, non-default temperature or repetition_penalty → `Temperature`, otherwise `Greedy`. Extracts `stop_sequences` from `StopConfig` (string or array), and wires all penalties, seed, and max_tokens into the config
