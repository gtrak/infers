...
- **compare.py**: CLI entry point for stage-by-stage comparison. Discovers engine dumps, loads DumpConfig, creates WeightLoader, and chains stages together computing reference outputs from `inputs` dict (each stage feeds the next). Intermediate results are always stored with `_gpu{idx}` suffix (e.g. `"attn.q_proj_raw_gpu0"`, `"attn.q_proj_raw_gpu1"`) so that each GPU's reference is computed independently — GPU 1 reads its own `q_proj_raw_gpu1` rather than reusing GPU 0's value. Accepts `--phase prefill|decode` to select which phase subdirectory to compare against (default: decode). Filters by layer type (full-attention vs GDN) and category (`--stages attn mlp gdn final`). Returns exit code 0 if all pass, 1 if any fail.
