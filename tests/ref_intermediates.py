...
def dequantize_int4_autogptq(
    ...
    The safetensors store transposed weight layout:
        ...
        qzeros:  [num_groups, K/8]
        scales:  [num_groups, N] or [num_groups, K] depending on axis
    ...
    For gate_proj with hidden_size=5120, intermediate_size=17408:
        ...
        After unpacking: [5120, 17408] = [K, N]
    """
    K_packed_dim = qweight.shape[0]
    N_dim = qweight.shape[1]
    K_dim = K_packed_dim * 8

    # Unpack weights: [K/8, N] -> [K, N] int4 values in [0, 15]
    ...
    w_grps = w_int4.reshape(num_groups, group_size, N_dim)
    z_grps = z_int4.reshape(num_groups, 1, N_dim)
